...
import { Upload, Trash2, Download, FileText, Receipt, FileSignature, Map, File } from 'lucide-react'
import { formatDate } from '@/lib/utils'
import type { PartyFile } from '@/components/admin/PartyDetail'
...
const CATEGORIES = [
  { key: 'contract', label: 'Contract', icon: <FileSignature size={15} /> },
  { key: 'invoice', label: 'Invoice', icon: <Receipt size={15} /> },
  { key: 'agreement', label: 'Agreement', icon: <FileSignature size={15} /> },
  { key: 'floor_plan', label: 'Floor Plan', icon: <Map size={15} /> },
...
export default function PartyFilesTab({ partyId, files }: Props) {
  ...
  async function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch(`/api/parties/${partyId}/files`, { method: 'POST', body: fd })
      if (!res.ok) {
  ...
  async function remove(fileId: string, name: string) {
    ...
    setError('')
    try {
      const res = await fetch(`/api/parties/${partyId}/files/${fileId}`, { method: 'DELETE' })
      if (!res.ok) {
        const json = await res.json().catch(() => ({}))
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      {files.length === 0 ? (
        <div className="py-12 text-center text-gray-400 text-sm">
          No files yet — upload contracts, invoices, or floor plans for this event.
        </div>
      ) : (
