...
import { Upload, Trash2, Download, FileText, Music, ListChecks, ScrollText, File } from 'lucide-react'
import { formatDate } from '@/lib/utils'
import type { CampFile } from '@/components/admin/CampDetail'
...
const CATEGORIES = [
  { key: 'packing_list', label: 'Packing List', icon: <ListChecks size={15} /> },
  { key: 'schedule', label: 'Schedule', icon: <FileText size={15} /> },
  { key: 'waiver', label: 'Waiver', icon: <ScrollText size={15} /> },
...
export default function CampFilesTab({ campId, files }: Props) {
  ...
  async function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch(`/api/camps/${campId}/files`, { method: 'POST', body: fd })
      if (!res.ok) {
  ...
  async function remove(fileId: string, name: string) {
    ...
    setError('')
    try {
      const res = await fetch(`/api/camps/${campId}/files/${fileId}`, { method: 'DELETE' })
      if (!res.ok) {
        const json = await res.json().catch(() => ({}))
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      {files.length === 0 ? (
        <div className="py-12 text-center text-gray-400 text-sm">
          No files yet — upload packing lists, schedules, waivers, or music for this camp.
        </div>
      ) : (
