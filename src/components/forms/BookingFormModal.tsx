...
export default function BookingFormModal({ onClose, rooms, partners = [], defaults, booking }: Props) {
  ...
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.title || !form.booking_date || !form.start_time || !form.end_time) {
  ...
  return (
    <SlideOver title={editing ? 'Edit Booking' : 'Add Booking'} onClose={onClose}>
        <form onSubmit={onSubmit} className="p-6 space-y-4">
          {error && <div className="p-3 rounded-lg bg-red-50 border border-red-200 text-red-700 text-sm">{error}</div>}

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Title *</label>
            <input value={form.title} onChange={e => set('title', e.target.value)} className={inputCls} placeholder="e.g. Private Lesson — Sarah T." />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Type</label>
              <select value={form.booking_type} onChange={e => set('booking_type', e.target.value)} className={inputCls}>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input type="date" value={form.booking_date} onChange={e => set('booking_date', e.target.value)} className={inputCls} />
            </div>
            <div>
